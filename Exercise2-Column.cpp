...
// This program sends a column (the fourth one).
...
int main(int argc, char** argv) {
    ...
    if (rank == 0) {
        ...
        MPI_Send(&A[0][3], 1, column, 1, 0, MPI_COMM_WORLD);
        MPI_Recv(&A[0][3], 1, column, 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

        printMatrix(rank, "received", N, (int*) A);

    } else if (rank == 1) {
        MPI_Recv(&B[0][3], 1, column, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        for (int i{0}; i < N; ++i) {
            for (int j{0}; j < N; ++j) {
                B[i][j] *= 10;
            }
        }
        MPI_Send(&B[0][3], 1, column, 0, 0, MPI_COMM_WORLD);
    }
